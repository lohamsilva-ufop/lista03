...
Arvore de busca binaria: cada nó possui um valor associado a ele e esse valor determina sua posicao no nó da arvore

Inserção: para cada nó pai, o valor maior que ele fica na direita e o valor menor ficam na esquerda
...
void inserir(int elemento, int indice, vector <no> &arvore){
    ...
    //se a arvore estiver cheia:
    ...
    } else if (elemento < arvore[indice].dado){ //verifica se o elemento passado é menor que elemento do nó corrente referente ao indice passado
    ...
    } else if (elemento > arvore[indice].dado){ //verifica se o elemento passado é maior que elemento do nó corrente referente ao indice passado
        if (arvore[indice].indice_direita == NULL){ //se for maior, insere na direita. Se o nó da direita nao tiver um indice apontando para o no direito
        //cria um registro
            no novo;
            novo.dado = elemento;
            novo.indice_direita = NULL;
        ...
        } else{ //se o indice da direita tiver elemento, ele verifica o proximo elemento dada a referencia do indice do proximo nó direito
            indice = arvore[indice].indice_direita;  // o indice recebe o indice da proximo nó direito
...
void busca (vector <no> arvore, int indice, int chave){
    ...
    } else if (chave == arvore[indice].dado){ //se é igual, ele encontrou o item
...
int main()
{
    vector <no> tree; //declaracao do vetor arvore do tipo nó
    ...
    while (opcao != 0){ //enquanto a opcao e diferente do 0, mostra ao usuario o menu:
        ...
        if (opcao == 1){
            ...
            cin >> elemento;
            inserir(elemento, 0, tree); //chama a funcao inserir, passando o elemento capturado, o indice para iniciar  busca e o vetor arvore do tipo nó
        } else if (opcao == 2){
            cout << "Digite o elemento a ser buscado na arvore:";
            cin >> elemento;
            busca(tree, 0, elemento); //chama a funcao busca, passando o vetor arvore do tipo nó, o indice para iniciar  busca e o elemento chave a ser buscado
        } else if (opcao == 3){
             imprimeArvore(tree); //chama a funcao que imprime a arvore passando o vetor arvore do tipo nó
        } else{
